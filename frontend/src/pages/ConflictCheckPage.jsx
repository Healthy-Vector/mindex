...
// severity는 API 명세서 #5 conflicts[].severity 값 그대로 — EXCLUDE 제약은 두 권리 중
// 하나라도 exclusivity=non_exclusive면 걸리지 않으므로(비독점은 원래 겹쳐도 되는 권리),
// 심각도는 이 3종류뿐이다. "비독점 상호 중복"은 애초에 서버가 충돌로 돌려주지 않는다.
const SEVERITY_META = {
...
function saveToast(saved, isFinal) {
  if (saved.historyStatus === "conflicted") {
...
export default function ConflictCheckPage() {
  ...
  // "데모: 충돌 있음/없음" 토글은 mock 상태에서만 의미가 있다 — 실 API가 붙으면 verify
  // 결과는 서버가 실제로 검사한 값이라 토글로 바꿔치기할 게 아니다.
  const [scenario, setScenario] = useState("conflict");
  const [result, setResult] = useState(initialResult ?? { hasConflict: false, checkedRows: 0, conflicts: [], candidate: null });
  ...
  function switchScenario(next) {
    setScenario(next);
    if (!payload) return;
    api.verifyContract({ ...payload, scenario: next }).then(setResult);
  }
  ...
  function handleSave() {
    ...
    const savePayload = USE_MOCK ? { ...payload, scenario } : payload;
    api
      .saveContract(savePayload)
  ...
  return (
    <div>
      <div className="mx-page-header">
        ...
        {USE_MOCK && (
          <div className="mx-seg conflict-demo-toggle">
            <button type="button" className={`mx-seg-opt${scenario === "conflict" ? " active" : ""}`} onClick={() => switchScenario("conflict")}>
              데모: 충돌 있음
            </button>
            <button type="button" className={`mx-seg-opt${scenario === "clean" ? " active" : ""}`} onClick={() => switchScenario("clean")}>
              데모: 충돌 없음
            </button>
          </div>
        )}
          ...
          <div key={i} className="conflict-group mx-mb-24">
            <div className="mx-flex-between mx-mb-20">
              <h4 className="mx-heading-card" style={{ margin: 0 }}>
                {c.existing.title} <span className="mx-muted mx-text-sm">— {c.existing.grantee}</span>
              </h4>
            ...
            <div className="mx-card mx-card-pad mx-mb-20">
              <ConflictTimeline
                ...
                incomingTitle={c.incoming.title ?? candidate?.title}
                overlap={c.overlap}
...
function CompareTable({ match, onShowQuote }) {
  ...
  const existingTerritory = match.existing.territory ?? match.territory;
  const incomingTerritory = match.incoming.territory ?? match.territory;
  const existingLegalRight = match.existing.legalRight ?? match.legalRight;
  const incomingLegalRight = match.incoming.legalRight ?? match.legalRight;
  const existingExploitationMode = match.existing.exploitationMode ?? match.exploitationMode;
  const incomingExploitationMode = match.incoming.exploitationMode ?? match.exploitationMode;
  const rows = [
    {
      key: "territory",
      label: "지역",
    ...
    { key: "period", label: "기간", existing: formatRange(match.existing.period), incoming: formatRange(match.incoming.period) },
    {
      key: "exclusivity",
      label: "독점 여부",
      existing: EXCLUSIVITY_LABEL[match.existing.exclusivity] ?? match.existing.exclusivity,
      incoming: EXCLUSIVITY_LABEL[match.incoming.exclusivity] ?? match.incoming.exclusivity,
    },
  ];
  ...
  return (
    <div className="mx-table-scroll">
      <table className="mx-table conflict-compare-table">
        ...
        <tbody>
          {rows.map((row) => {
            const quotes = row.key === "territory" || row.key === "exclusivity" ? { existing: match.existing.evidence, incoming: match.incoming.evidence } : null;
            return (
              <tr
                key={row.key}
