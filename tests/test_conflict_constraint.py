...
# ─────────────────────────────────────────────────────────────
#
# 기존/신규 모두 동일 tenant · ip · territory(JP) · period(겹침) · exclusive 전제.
# 달라지는 것은 두 판정축뿐이다.
@pytest.mark.parametrize(
    ...
    [
        ...
        # 이게 D-13(단일 축) 구조에서는 표현조차 안 되던 케이스다.
...
def test_hierarchy_overlap(conn, cur, make_grant, existing, incoming, blocked, why):
    ...
    if blocked:
        ...
        make_grant(legal_right=incoming[0], exploitation_mode=incoming[1])  # 통과해야 한다


# ─────────────────────────────────────────────────────────────
# 나머지 판정축 — 계층 도입으로 회귀하지 않았는지
# ─────────────────────────────────────────────────────────────
def test_different_territory_is_allowed(make_grant):
    make_grant(territory="JP")
    make_grant(territory="KR")


def test_adjacent_period_is_allowed(make_grant):
    """EN-B01 — 반열림 구간이라 12/31 종료와 1/1 시작은 겹치지 않는다."""
    make_grant(period="[2026-01-01,2027-01-01)")
    make_grant(period="[2027-01-01,2028-01-01)")


def test_one_day_overlap_is_blocked(conn, make_grant):
...
def test_both_non_exclusive_is_allowed(make_grant):
    """비독점끼리는 겹쳐도 정상이다 — EXCLUDE의 WHERE절이 걸러낸다."""
    make_grant(exclusivity="non_exclusive")
    make_grant(exclusivity="non_exclusive")


def test_terminated_grant_does_not_block(make_grant, cur):
...
def test_exclusivity_xor_caught_by_trigger(conn, make_grant, existing_excl, incoming_excl):
    make_grant(exclusivity=existing_excl)
    with pytest.raises(psycopg2.errors.ExclusionViolation) as excinfo:
        make_grant(exclusivity=incoming_excl)
    # EXCLUDE가 아니라 트리거가 잡아야 한다 — 담당이 XOR로 배타 분할돼 있다
    assert constraint_of(excinfo) == "no_exclusivity_conflict"
    conn.rollback()


def test_exclusivity_xor_sees_hierarchy(conn, make_grant):
...
# 비정규화 span — P-4 방어
# ─────────────────────────────────────────────────────────────
def test_span_is_derived_not_trusted(cur, ctx, make_candidate):
    """앱이 span을 직접 넣어도 트리거가 코드 기준으로 덮어쓴다.

    이걸 허용하면 "코드는 SVOD인데 span은 THEATRICAL"인 행을 만들 수 있고,
    그러면 EXCLUDE가 정상 동작하면서 충돌을 조용히 놓친다.
    """
    candidate_id = make_candidate(legal_right="TRANSMISSION", exploitation_mode="SVOD")
    cur.execute(
        """
        ...
        VALUES (%s, %s, %s, %s,
                ...
                '[2027-01-01,2028-01-01)'::daterange, 'exclusive', 'tester')
        RETURNING legal_right_span, exploitation_mode_span
        """,
        (ctx["contract_id"], ctx["document_id"], candidate_id, ctx["ip_id"]),
    )
...
def test_unknown_axis_code_is_rejected(conn, cur, ctx, make_candidate):
    """정의되지 않은 판정축 코드는 FK가 막는다."""
    make_candidate()
    with pytest.raises(psycopg2.errors.ForeignKeyViolation):
        cur.execute(
            "INSERT INTO rights_grant_candidate "
            "  (contract_id, document_id, ip_id, legal_right) "
            "VALUES (%s, %s, %s, 'NOT_A_RIGHT')",
            (ctx["contract_id"], ctx["document_id"], ctx["ip_id"]),
        )
    conn.rollback()


def test_contract_delete_survives_diamond_cascade(cur, ctx, make_grant):
    """D-26이 미검증으로 남긴 위험 — 실측 결과 PostgreSQL 16.14에서 문제없다.

    rights_grant.source_candidate_id는 Evidence Anchoring 보존을 위해 CASCADE가
    아닌데(D-25) rights_grant_candidate.contract_id는 CASCADE다. 두 캐스케이드
    경로의 처리 순서를 PostgreSQL이 보장하지 않으므로, candidate가 먼저 지워지면
    RESTRICT에 걸려 DELETE 전체가 실패할 수 있다는 우려가 있었다.
    """
    grant_id = make_grant()
