...
> PIN 세션이 필요합니다. snippet이 **계약서 원문 인용**이라 같은 원문을 돌려주는 9번과 기준을 맞췄습니다(D-40).
> PIN은 "이 설치에 접근할 자격"만 확인합니다 — `team`은 PIN 관리 전용 테이블이고 `team_id`를 도메인 테이블에 전파하지 않으므로(D-29/D-30, 단일사 온프렘) 토큰의 팀 정보는 조회 범위를 가르지 않습니다.
